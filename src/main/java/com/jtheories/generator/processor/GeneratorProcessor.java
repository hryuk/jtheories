...
public class GeneratorProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        for (TypeElement annotation : annotations) {
            ...
            Collection<TypeElement> annotatedInterfaces = annotatedElements
                    .stream()
                    .map(TypeElement.class::cast)
                    .collect(Collectors.toList());

            for(TypeElement annotatedInterface:annotatedInterfaces) {
                ExecutableElement defaultMethod = annotatedInterface.getEnclosedElements()
                        .stream()
                ...
                if(defaultMethod.getParameters().size()!=1){
                    fatal(String.format("Default generate() definition receiving more than one parameter in %s",annotatedInterface.getSimpleName()));
                }
                ...
                CodeBlock codeBlock = CodeBlock.builder()
                        .addStatement("return $T.super.generate($N)", ClassName.get(annotatedInterface.asType()),ParameterSpec.get(defaultMethod.getParameters().get(0)))
                        .build();
                ...
                String generatedClassName = TypeName.get(getGenerateReturnType(annotatedInterface)).toString();
                String generatorPackage = annotatedElementClassName.substring(0,annotatedElementClassName.lastIndexOf('.'));
                String generatorClassName = String.format("Arbitrary%sGenerator",generatedClassName.substring(generatedClassName.lastIndexOf('.')+1));

                TypeSpec arbitraryObject = TypeSpec.classBuilder(generatorClassName)
                        .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
                        .addSuperinterface(ParameterizedTypeName.get(ClassName.get(Generator.class), ClassName.get(getGenerateReturnType(annotatedInterface))))
                        .addSuperinterface(ClassName.get(annotatedInterface.asType()))
                        .addAnnotation(generatedAnnotaion)
                        .addMethod(method)
                        .build();

                JavaFile javaFile = JavaFile.builder(generatorPackage, arbitraryObject)
                        .build();
