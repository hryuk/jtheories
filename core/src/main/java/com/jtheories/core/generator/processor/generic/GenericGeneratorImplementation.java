...
public class GenericGeneratorImplementation extends GeneratorImplementation {
  ...
  private TypeSpec createArbitraryGenerator() {
    ...
    generatorMethods.add(new GenericGenerateConstrainedMethod(information).getConstrainedMethod());

    var typeBuilder =
        TypeSpec.classBuilder(information.getImplementerName())
            .addModifiers(Modifier.PUBLIC, Modifier.FINAL)
            .addSuperinterface(information.getClassName())
            .addSuperinterface(
                ParameterizedTypeName.get(
                    ClassName.get(Generator.class), information.getReturnClassName()))
