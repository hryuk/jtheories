...
public class ParameterAssignment {
	...
	private CodeBlock generateAnnotationParameters(VariableElement parameter) {
		...
		return CodeBlock.of(
			...
			parameter
				...
				.stream()
				.map(this::generateAnnotationParameter)
				.collect(CodeBlock.joining(",$W"))
		);
	}

	private CodeBlock generateAnnotationParameter(AnnotationMirror annotationMirror) {
		return CodeBlock.of(
			"$T.builder().annotation($T.class)$L.build()",
	...
	private CodeBlock generatePlainTypeAssignment(
		...
		return CodeBlock.of(
			...
			TypeArgument.class,
			parameterType,
			this.generateAnnotationParameters(parameter)
		);
	}
